...
arduinoFFT::arduinoFFT(SAMPLE_T *vReal, SAMPLE_T *vImag, uint16_t samples,
                       SAMPLE_T samplingFrequency) { // Constructor
  this->_vReal = vReal;
  this->_vImag = vImag;
...
void arduinoFFT::Compute(SAMPLE_T *vReal, SAMPLE_T *vImag, uint16_t samples,
                         uint8_t dir) {
#warning("This method is deprecated and may be removed on future revisions.")
  Compute(vReal, vImag, samples, Exponent(samples), dir);
}

void arduinoFFT::Compute(
    uint8_t dir) { // Computes in-place complex-to-complex FFT /
  // Reverse bits /
  uint16_t j = 0;
  ...
  for (uint8_t l = 0; (l < this->_power); l++) {
    ...
    for (j = 0; j < l1; j++) {
      for (uint16_t i = j; i < this->_samples; i += l2) {
        uint16_t i1 = i + l1;
        SAMPLE_T t1 = u1 * this->_vReal[i1] - u2 * this->_vImag[i1];
        SAMPLE_T t2 = u1 * this->_vImag[i1] + u2 * this->_vReal[i1];
        this->_vReal[i1] = this->_vReal[i] - t1;
        this->_vImag[i1] = this->_vImag[i] - t2;
        this->_vReal[i] += t1;
        this->_vImag[i] += t2;
      }
      SAMPLE_T z = ((u1 * c1) - (u2 * c2));
      u2 = ((u1 * c2) + (u2 * c1));
      u1 = z;
    }
#ifdef __AVR__
...
void arduinoFFT::Compute(
    SAMPLE_T *vReal, SAMPLE_T *vImag, uint16_t samples, uint8_t power,
    uint8_t dir) { // Computes in-place complex-to-complex FFT
// Reverse bits
#warning("This method is deprecated and may be removed on future revisions.")
  uint16_t j = 0;
  ...
  for (uint8_t l = 0; (l < power); l++) {
    ...
    for (j = 0; j < l1; j++) {
      for (uint16_t i = j; i < samples; i += l2) {
        uint16_t i1 = i + l1;
        SAMPLE_T t1 = u1 * vReal[i1] - u2 * vImag[i1];
        SAMPLE_T t2 = u1 * vImag[i1] + u2 * vReal[i1];
        vReal[i1] = vReal[i] - t1;
        vImag[i1] = vImag[i] - t2;
        vReal[i] += t1;
        vImag[i] += t2;
...
void arduinoFFT::ComplexToMagnitude() { // vM is half the size of vReal and
                                        // vImag
  for (uint16_t i = 0; i < this->_samples; i++) {
...
void arduinoFFT::ComplexToMagnitude(
    SAMPLE_T *vReal, SAMPLE_T *vImag,
    uint16_t samples) { // vM is half the size of vReal and vImag
#warning("This method is deprecated and may be removed on future revisions.")
  for (uint16_t i = 0; i < samples; i++) {
...
void arduinoFFT::DCRemoval(SAMPLE_T *vData, uint16_t samples) {
// calculate the mean of vData
#warning("This method is deprecated and may be removed on future revisions.")
  SAMPLE_T mean = 0;
  for (uint16_t i = 0; i < samples; i++) {
...
void arduinoFFT::Windowing(uint8_t windowType,
                           uint8_t dir) { // Weighing factors are computed once
                                          // before multiple use of FFT
  // The weighing function is symetric; half the weighs are recorded
  SAMPLE_T samplesMinusOne = (SAMPLE_T(this->_samples) - 1.0);
  for (uint16_t i = 0; i < (this->_samples >> 1); i++) {
    SAMPLE_T indexMinusOne = SAMPLE_T(i);
    SAMPLE_T ratio = (indexMinusOne / samplesMinusOne);
    SAMPLE_T weighingFactor = 1.0;
...
void arduinoFFT::Windowing(
    SAMPLE_T *vData, uint16_t samples, uint8_t windowType,
    uint8_t
        dir) { // Weighing factors are computed once before multiple use of FFT
// The weighing function is symetric; half the weighs are recorded
#warning("This method is deprecated and may be removed on future revisions.")
  SAMPLE_T samplesMinusOne = (SAMPLE_T(samples) - 1.0);
  for (uint16_t i = 0; i < (samples >> 1); i++) {
    SAMPLE_T indexMinusOne = SAMPLE_T(i);
    SAMPLE_T ratio = (indexMinusOne / samplesMinusOne);
    SAMPLE_T weighingFactor = 1.0;
...
void arduinoFFT::MajorPeak(SAMPLE_T *f, SAMPLE_T *v) {
  SAMPLE_T maxY = 0;
  uint16_t IndexOfMaxY = 0;
  // If sampling_frequency = 2 * max_frequency in signal,
  // value would be stored at position samples/2
...
SAMPLE_T arduinoFFT::MajorPeak(SAMPLE_T *vD, uint16_t samples,
                               SAMPLE_T samplingFrequency) {
#warning("This method is deprecated and may be removed on future revisions.")
  SAMPLE_T maxY = 0;
  uint16_t IndexOfMaxY = 0;
  // If sampling_frequency = 2 * max_frequency in signal,
...
void arduinoFFT::MajorPeak(SAMPLE_T *vD, uint16_t samples,
                           SAMPLE_T samplingFrequency, SAMPLE_T *f,
                           SAMPLE_T *v) {
#warning("This method is deprecated and may be removed on future revisions.")
  SAMPLE_T maxY = 0;
  uint16_t IndexOfMaxY = 0;
...
SAMPLE_T arduinoFFT::MajorPeakParabola() {
  SAMPLE_T maxY = 0;
  uint16_t IndexOfMaxY = 0;
  // If sampling_frequency = 2 * max_frequency in signal,
  // value would be stored at position samples/2
...
void arduinoFFT::Parabola(SAMPLE_T x1, SAMPLE_T y1, SAMPLE_T x2, SAMPLE_T y2,
                          SAMPLE_T x3, SAMPLE_T y3, SAMPLE_T *a, SAMPLE_T *b,
                          SAMPLE_T *c) {
  SAMPLE_T reversed_denom = 1 / ((x1 - x2) * (x1 - x3) * (x2 - x3));
...
void arduinoFFT::Swap(SAMPLE_T *x, SAMPLE_T *y) {
  SAMPLE_T temp = *x;
  *x = *y;
  *y = temp;
}
